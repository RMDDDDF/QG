...
{
	LinkedList* L = head;//传入的是指向链表头指针的指针
...
{
	ElemType* e = NULL;
	if (L==NULL||L->next == NULL)//第一个节点为空或者头指针为空
	{
